  ...
  void _submitForm() {
    if (_formKey.currentState!.validate()) {
      ...
      if (_urlController.text.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Please enter a URL for a user to learn more"),
          ),
        );
        return;
      }
      ...
      context.read<CreateEventBloc>().add(
        SubmitNewEvent(event: event, imageFile: _selectedImage),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: CustomScrollView(
        slivers: [
          SliverAppBar.large(title: Text("Create an event")),
          SliverToBoxAdapter(
