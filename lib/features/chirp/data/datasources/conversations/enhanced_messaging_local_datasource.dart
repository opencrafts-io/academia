...
abstract class EnhancedMessagingLocalDataSource {
  ...
  Future<Either<Failure, List<EnhancedConversationData>>>
  ...
  Future<Either<Failure, EnhancedMessageData>> addMessage(
  ...
  Future<Either<Failure, WebSocketConnectionData?>> getConnectionState(
    String connectionId,
  );
  ...
  Future<Either<Failure, void>> queueOfflineMessage(
    String conversationId,
    String content,
  );
...
class EnhancedMessagingLocalDataSourceImpl
    implements EnhancedMessagingLocalDataSource {
      ...
      await localDB.transaction(() async {
        ...
        for (final conversation in conversations) {
          ...
          final conversationData = EnhancedConversationData(
            ...
            updatedAt:
                DateTime.tryParse(conversation.updatedAt) ?? DateTime.now(),
            createdAt:
                DateTime.tryParse(conversation.createdAt) ?? DateTime.now(),
          );

          await localDB
              .into(localDB.enhancedConversationTable)
  ...
  getCachedConversations() async {
    try {
      final conversations =
          await (localDB.select(localDB.enhancedConversationTable)..orderBy([
                (c) => OrderingTerm(
      ...
      return Left(
        CacheFailure(error: e, message: 'Failed to load conversations.'),
      );
    }
  }
  ...
  @override
  Stream<List<EnhancedConversationData>> getConversationsStream() {
    return (localDB.select(localDB.enhancedConversationTable)..orderBy([
          (c) => OrderingTerm(
            expression: c.lastMessageAt ?? c.apiUpdatedAt ?? c.updatedAt,
            mode: OrderingMode.desc,
          ),
  ...
  Future<Either<Failure, void>> cacheMessages(
    ...
    try {
        ...
        for (final message in messages) {
          final messageData = EnhancedMessageTableCompanion(
            ...
            imageUrl: const Value(
              null,
            ), // Would need to extract from attachments
          ...
          final insertedMessage = await localDB
              .into(localDB.enhancedMessageTable)
          ...
          for (final attachment in message.attachments) {
            ...
            await localDB
                .into(localDB.messageAttachmentsTable)
      ...
      if (e.toString().contains('no such table')) {
        _logger.w(
          'Enhanced messaging tables not created yet, skipping cache operation',
        );
        return const Right(null);
      }

      return Left(
        CacheFailure(error: e, message: 'Failed to save messages locally.'),
      );
    }
  }
  ...
  Future<Either<Failure, List<EnhancedMessageData>>> getCachedMessages(
    ...
    try {
      ...
      final messages =
          await (localDB.select(localDB.enhancedMessageTable)
      ...
      if (e.toString().contains('no such table')) {
        _logger.w(
          'Enhanced messaging tables not created yet, returning empty messages',
        );
        return const Right([]);
      }

      return Left(CacheFailure(error: e, message: 'Failed to load messages.'));
    }
  }
  ...
  Stream<List<EnhancedMessageData>> getMessagesStream(String conversationId) {
    try {
      ...
      _logger.w(
        'Enhanced messaging table not available, returning empty stream',
      );
  ...
  Future<Either<Failure, EnhancedMessageData>> addMessage(
    ...
    try {
      ...
      if (e.toString().contains('no such table')) {
        _logger.w(
          'Enhanced messaging tables not created yet, skipping add operation',
        );
      ...
      return Left(CacheFailure(error: e, message: 'Failed to save message.'));
    }
  }
  ...
  Future<Either<Failure, void>> updateMessage(MessageApiModel message) async {
    try {
      await (localDB.update(
        localDB.enhancedMessageTable,
      )..where((m) => m.apiMessageId.equals(message.id))).write(
      ...
      return Left(CacheFailure(error: e, message: 'Failed to update message.'));
    }
  }
  ...
  Future<Either<Failure, void>> deleteMessage(int messageId) async {
    try {
      await (localDB.delete(
        localDB.enhancedMessageTable,
      )..where((m) => m.apiMessageId.equals(messageId))).go();

      return const Right(null);
    } catch (e) {
      _logger.e('Failed to delete message: $e');
      return Left(CacheFailure(error: e, message: 'Failed to delete message.'));
    }
  }

  @override
  Future<Either<Failure, EnhancedMessageData?>> getMessageById(
    int messageId,
  ) async {
    try {
      final message = await (localDB.select(
        localDB.enhancedMessageTable,
      )..where((m) => m.apiMessageId.equals(messageId))).getSingleOrNull();

      return Right(message);
    } catch (e) {
      _logger.e('Failed to get message by ID: $e');
      return Left(CacheFailure(error: e, message: 'Failed to load message.'));
    }
  }

  @override
  Future<Either<Failure, void>> cacheUser(
    ConversationParticipantApiModel user,
  ) async {
    return _cacheUserInternal(user);
  }

  Future<Either<Failure, void>> _cacheUserInternal(
    ConversationParticipantApiModel user,
  ) async {
    try {
      ...
      await localDB
          ...
          .insertOnConflictUpdate(userData);
      return const Right(null);
    } catch (e) {
      _logger.e('Failed to cache user: $e');
      return Left(CacheFailure(error: e, message: 'Failed to save user data.'));
    }
  }
  ...
  Future<Either<Failure, ChirpUserData?>> getCachedUser(String userId) async {
    try {
      ...
      return Left(CacheFailure(error: e, message: 'Failed to load user data.'));
    }
  }
  ...
  Future<Either<Failure, void>> saveDraft(
    ...
    try {
      ...
      await localDB
          .into(localDB.draftMessagesTable)
          .insertOnConflictUpdate(draftData);
      return const Right(null);
    } catch (e) {
      ...
      if (e.toString().contains('no such table')) {
        _logger.w(
          'Draft messages table not created yet, skipping save operation',
        );
        return const Right(null);
      }

      return Left(CacheFailure(error: e, message: 'Failed to save draft.'));
    }
  }

  @override
  Future<Either<Failure, DraftMessageData?>> getDraft(
    String conversationId,
  ) async {
    try {
      final draft =
          await (localDB.select(localDB.draftMessagesTable)
      ...
      return Left(CacheFailure(error: e, message: 'Failed to load draft.'));
    }
  }
  ...
  Future<Either<Failure, void>> deleteDraft(String conversationId) async {
    try {
      await (localDB.delete(
        localDB.draftMessagesTable,
      )..where((d) => d.conversationId.equals(conversationId))).go();
      ...
      if (e.toString().contains('no such table')) {
        _logger.w(
          'Draft messages table not created yet, skipping delete operation',
        );
        return const Right(null);
      }

      return Left(CacheFailure(error: e, message: 'Failed to delete draft.'));
    }
  }
  ...
  Future<Either<Failure, void>> updateConnectionState(
    ...
    try {
      ...
      await localDB
          .into(localDB.webSocketConnectionTable)
      ...
      return Left(
        CacheFailure(error: e, message: 'Failed to update connection state.'),
      );
    }
  }
  ...
  Future<Either<Failure, WebSocketConnectionData?>> getConnectionState(
    ...
    try {
      final connection = await (localDB.select(
        localDB.webSocketConnectionTable,
      )..where((c) => c.connectionId.equals(connectionId))).getSingleOrNull();

      return Right(connection);
    } catch (e) {
      _logger.e('Failed to get connection state: $e');
      return Left(
        CacheFailure(error: e, message: 'Failed to load connection state.'),
      );
    }
  }
  ...
  Future<Either<Failure, void>> invalidateConversationsCache() async {
    try {
      ...
      return Left(
        CacheFailure(error: e, message: 'Failed to clear conversations cache.'),
      );
    }
  }

  @override
  Future<Either<Failure, void>> invalidateMessagesCache(
    String conversationId,
  ) async {
    try {
      await (localDB.delete(
      ...
      return Left(
        CacheFailure(error: e, message: 'Failed to clear messages cache.'),
      );
    }
  }
  ...
  Future<Either<Failure, void>> clearAllCache() async {
    try {
      ...
      return Left(CacheFailure(error: e, message: 'Failed to clear cache.'));
    }
  }
  ...
  Future<Either<Failure, void>> markMessagesAsRead(
    ...
    try {
      for (final messageId in messageIds) {
        await (localDB.update(
          localDB.enhancedMessageTable,
        )..where((m) => m.apiMessageId.equals(messageId))).write(
      ...
      await (localDB.update(
        localDB.enhancedConversationTable,
      )..where((c) => c.conversationId.equals(conversationId))).write(
      ...
      return Left(
        CacheFailure(error: e, message: 'Failed to mark messages as read.'),
      );
    }
  }
  ...
  Future<Either<Failure, int>> getUnreadCount(
    ...
    try {
      final result =
          await (localDB.selectOnly(localDB.enhancedMessageTable)
                ..addColumns([localDB.enhancedMessageTable.id.count()])
                ..where(
                  localDB.enhancedMessageTable.conversationId.equals(
                        conversationId,
                      ) &
                      localDB.enhancedMessageTable.senderId
      ...
      return Left(
        CacheFailure(error: e, message: 'Failed to get unread count.'),
      );
    }
  }
  ...
  Future<Either<Failure, List<DraftMessageData>>> getOfflineQueue() async {
    try {
      ...
      return Left(
        CacheFailure(error: e, message: 'Failed to load offline messages.'),
      );
    }
  }
  ...
  Future<Either<Failure, void>> clearOfflineQueue() async {
    try {
      ...
      return Left(
        CacheFailure(error: e, message: 'Failed to clear offline messages.'),
      );
    }
  }
