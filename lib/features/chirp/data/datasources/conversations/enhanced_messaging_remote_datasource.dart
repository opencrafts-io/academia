...
abstract class EnhancedMessagingRemoteDataSource {
  ...
  Future<Either<Failure, ConversationApiModel>> getConversationDetails(
  ...
  Future<Either<Failure, Map<String, dynamic>>> getConversationInfo(
    String conversationId,
  );
}

class EnhancedMessagingRemoteDataSourceImpl
    implements EnhancedMessagingRemoteDataSource {
  ...
  Future<Either<Failure, ConversationApiModel>> getConversationDetails(
    ...
    try {
      ...
      final response = await dioClient.dio.get(
        '/$servicePath/conversations/$conversationId/',
      );
      ...
      if (response.statusCode == 200) {
        ...
        _logger.e(
          'Failed to fetch conversation details: ${response.statusCode}',
        );
        return Left(
          ServerFailure(
            error: Exception('HTTP ${response.statusCode}'),
      ...
      _logger.e(
        'DioException while fetching conversation details: ${e.message}',
      );
      return Left(
        _handleDioException(e, 'Failed to load conversation details'),
      );
    } catch (e) {
  ...
  Future<Either<Failure, List<MessageApiModel>>> getMessages(
    ...
    try {
      _logger.d(
        'Fetching messages for conversation: $conversationId (page: $page, size: $pageSize)',
      );
      ...
      if (response.statusCode == 200) {
        ...
        final messages = messagesJson.map((json) {
          ...
          if (!messageJson.containsKey('has_attachment')) {
            messageJson['has_attachment'] =
                messageJson.containsKey('attachments') &&
  ...
  Future<Either<Failure, void>> markMessagesAsRead(
    ...
    try {
      _logger.d(
        'Marking messages as read for conversation: $conversationId (temporarily disabled)',
      );
      ...
      _logger.w(
        'Mark-as-read temporarily disabled due to backend Django ORM query issue',
      );
  ...
  Future<Either<Failure, MessageApiModel>> _sendMessageWithAttachment({
    ...
    try {
      _logger.d(
        'Sending message with attachment for conversation: $conversationId',
      );
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        _logger.e(
          'Failed to send message with attachment: ${response.statusCode}',
        );
        return Left(
          ServerFailure(
            error: Exception('HTTP ${response.statusCode}'),
            message:
                'File uploaded but failed to send message. Please try again.',
          ),
        );
      }
    } on DioException catch (e) {
      _logger.e(
        'DioException while sending message with attachment: ${e.message}',
      );
      return Left(
        _handleDioException(e, 'File uploaded but failed to send message'),
      );
    }
  }
  ...
  Future<Either<Failure, Map<String, dynamic>>> getConversationInfo(
    ...
    try {
      ...
      return Left(
        _handleDioException(e, 'Failed to load conversation information'),
      );
    } catch (e) {
      ...
      return Left(
        NetworkFailure(
          error: e,
          message:
              'An unexpected error occurred while loading conversation information.',
        ),
      );
    }
  }
  ...
  Failure _handleDioException(DioException e, String defaultMessage) {
    switch (e.type) {
      ...
      case DioExceptionType.badResponse:
        ...
        switch (statusCode) {
          case 400:
            ...
            return ValidationFailure(message: message, error: e);
          ...
          default:
            return ServerFailure(error: e, message: defaultMessage);
        }

      case DioExceptionType.cancel:
        return NetworkFailure(error: e, message: 'Request was cancelled.');
      ...
      default:
        return NetworkFailure(error: e, message: defaultMessage);
    }
  }
}
